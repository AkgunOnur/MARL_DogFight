...
def train(train_timesteps: int, n_procs: int, folder_postfix: str, scenario: str, visualize: bool):
    ...
    for index, current_model in enumerate(model_list): 
        for curriculum in curriculum_list:
            ...
            if curriculum == "level2":                
                previous_level = scenario + "_" + model_names[index] + "_" + curriculum_list[0] + "_" + folder_postfix
                model = current_model.load(model_dir + "/" +  previous_level + "/best_model", verbose=1) # + "/best_model"
