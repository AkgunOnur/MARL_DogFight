    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if mean_reward > self.best_mean_reward:
                self.best_rewards_list.append(mean_reward)
                if self.verbose > 0:
...
class StopTrainingOnNoRemarkableImprovement(BaseCallback):
    ...
    def __init__(self, max_no_improvement_evals: int, min_evals: int = 0, check_percentage=0.9, verbose: int = 0):
        super(StopTrainingOnNoRemarkableImprovement, self).__init__(verbose=verbose)
        self.max_no_improvement_evals = max_no_improvement_evals
        self.min_evals = min_evals
        self.last_best_mean_reward = -np.inf
        self.no_improvement_evals = 0
        self.check_percentage = check_percentage
...
# -17 -15 1
        ...
        if len(self.parent.best_rewards_list) > 5:
            continue_training = False
            #print ("Last rewards: ", self.parent.best_rewards_list[-3:])
            if self.parent.best_rewards_list[-1] > 0:
                if self.check_percentage*self.parent.best_rewards_list[-1] > self.parent.best_rewards_list[-2] and \
                    self.check_percentage*self.parent.best_rewards_list[-1] > self.parent.best_rewards_list[-3]:
                    continue_training = True
            else:
                if (2-self.check_percentage) * self.parent.best_rewards_list[-1] > self.parent.best_rewards_list[-2] and \
                   (2-self.check_percentage) * self.parent.best_rewards_list[-1] > self.parent.best_rewards_list[-3]:
                    continue_training = True

        if continue_training == False and self.verbose > 0:
            print(
                f"Stopping training because there was no remarkable improvement in the last evaluations"
            )
            print ("Last rewards: ", self.parent.best_rewards_list[-3:])
